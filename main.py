...
@app.route("/home_student", methods=['GET'])
...
def stats():
    ...
    try:
        ...
        mycursor.execute(qry)
        organ_list = [organ[0] for organ in mycursor.fetchall()]

        A.clear()
        B.clear()
        for organ in organ_list:
            qry = f"select count(*) from Transaction inner join Donor on Donor.Donor_ID = Transaction.Donor_ID where Organ_donated = '{organ}' and Status = 1"
            mycursor.execute(qry)
            a = mycursor.fetchone()
            A.append(a[0])
            qry = f"select count(*) from Transaction inner join Donor on Donor.Donor_ID = Transaction.Donor_ID where Organ_donated = '{organ}' and Status = 0"
            mycursor.execute(qry)
            b = mycursor.fetchone()
            B.append(b[0])
